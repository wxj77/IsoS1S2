function [isolated_s1_rate,isolated_s2_rate,isolated_s1_spect,isolated_s2_spect,s1_rate,s2_rate,s1_spect,s2_spect,livetime,isolated_livetime,allquiet_livetime] = IsolateS1S2s_OneS4(ee_merge,MaxDriftLength)
		...
		elseif jj ==length( ee_merge.rqs.pulse_start_samples)
			...
			if ((delta_t_p1 >MaxDriftLength) && (delta_t_m1 >MaxDriftLength))
				if  (ee_merge.rqs.pulse_classification(listOfS1S2s(jj)) == 2)
					temp_list = [];					
					for kk =1:(length(listOfS4s)-1)
						temp_list = [temp_list, abs(ee_merge.rqs.pulse_start_samples(listOfS4s(kk))-ee_merge.rqs.pulse_start_samples(listOfS1S2s(jj)))];
					end
					temp_list = temp_list<MaxDriftLength;
